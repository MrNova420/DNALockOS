...
class AuthenticationService:
    ...
    def generate_challenge(self, request: ChallengeRequest) -> ChallengeResponse:
        ...
        try:
            ...
            if not dna_key.is_valid() or dna_key.is_expired():
                return ChallengeResponse(
                    success=False,
                    error_message="Key is invalid or expired"
                )
        ...
        try:
            ...
            if challenge_id not in self._challenges:
                return AuthenticationResponse(
                    success=False,
                    error_message="Invalid challenge ID",
            ...
            if challenge_data["used"]:
                return AuthenticationResponse(
                    success=False,
                    error_message="Challenge already used",
                    timestamp=datetime.now(timezone.utc)
                )
    ...
    def _verify_challenge_response(
        ...
        try:
            ...
            # Create verify key
            verify_key = Ed25519VerifyKey.from_bytes(
                dna_key.cryptographic_material.public_key
            )
